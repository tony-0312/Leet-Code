...
class Count{
	...
	static int countHillValley(int[] nums){
		...
		for(int i = 1; i < nonDup.size() - 1; i++){
			...
			//System.out.print(nonDup.get(i) + " ");
			
			if(nonDup.get(i) > nonDup.get(i-1) && nonDup.get(i) > nonDup.get(i+1)){
				countHills++;
			}
			else if(nonDup.get(i) < nonDup.get(i-1) && nonDup.get(i) < nonDup.get(i+1)){
				countValley++;
			}
			
		}
		
		return countHills + countValley;
	...
	public static void main(String[] args){
		...
		System.out.print(countHillValley(nums));
