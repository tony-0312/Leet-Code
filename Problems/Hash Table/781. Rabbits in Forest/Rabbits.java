...
class Rabbits{
	...
	static int findMinRabbits(int[] answers){
		...
		for(Map.Entry<Integer, Integer> entrySet : freq.entrySet()){
			...
			int otherRabbits = entrySet.getKey();
			int freqClaim = entrySet.getValue();
			
			int groupOfRabbits = (int)Math.ceil( freqClaim / ((otherRabbits + 1) * 1.0));
			minRabbits += groupOfRabbits * (otherRabbits + 1);
	...
	public static void main(String[] args){
		...
		System.out.println(findMinRabbits(answers));
	}
