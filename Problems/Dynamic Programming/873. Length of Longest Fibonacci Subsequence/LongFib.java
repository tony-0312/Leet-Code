...
class LongFib{
    static int findLongestFibSequence(int[] arr){
        int longSubSeq = 0;
        int size = arr.length;
        ...
        for(int i = 0; i < size; i++){
            for(int j = 0; j < i; j++){
                ...
                if(numInd.containsKey(prevValue)){
                    int temp = numInd.get(prevValue);
                    if(temp < j){
                        dp[j][i] = Math.max(dp[j][i], dp[temp][j] + 1);
                        longSubSeq = Math.max(longSubSeq, dp[j][i]);
                    }
                }
            }
        }

        return longSubSeq > 2 ? longSubSeq : 0;
    }
    public static void main(String[] args) {
        int[] nums = {1,2,3,4,5,6,7,8};
        System.out.println(findLongestFibSequence(nums));
    }
}